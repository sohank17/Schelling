...
def initialize(rows, columns,empty_percent,population_percent):
    ...
    return grid, grid_dict, empty_list_key
...
iterations=50
k= 4
initial_plot='First_Policy1'
final_plot='Final_Policy1'
graph_plot='graph_Policy1'


grid, grid_dict, empty_list_key = initialize(rows, columns,empty_percent,population_percent)
plot(rows, columns, 'First', initial_plot)
happy_agents = update(iterations,k,rows,columns)
print("Happy Agents:", happy_agents[-1][0])
plot_graph(graph_plot)
plot(rows, columns, 'Last', final_plot)
